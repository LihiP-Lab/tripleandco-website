...
- Readiness score page: `/ai-revenue-readiness-score` (component `src/components/ReadinessScore.tsx`). Shared links use `?r=<20-digit code of 0-3>&d=<domain>`; any valid code for API tests is e.g. `11111111111111111111`. Shared loads must show "From a shared link, not verified in this session" (never "Measured live") and fire no completion beacon; a fresh completion logs exactly one `readiness_score_completed` JSON line.
- `/api/readiness-report` has two independent in-memory rate limits (reset on server restart): 5/min per IP and 3/hour per recipient email (case-insensitive). To isolate the per-recipient limit, send each request with a distinct `x-forwarded-for` header. Check order is validation (400) → recipient limit (429) → RESEND_API_KEY (503 "Report delivery is not available right now." when unset, which is the honest local behavior).
- The QA route sweep asserts title ≤65 chars AND description 70-165 chars, and assertions short-circuit: fixing one failure may expose the next, so re-run the sweep after each metadata fix.
