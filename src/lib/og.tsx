...
/**
 * Branded 1200x630 scorecard for a shared AI Visibility Checker result:
 * big score out of 100, grade, and the checked domain.
 */
export async function ogVisibilityScoreCard({ domain, score, grade, headers }: OgScoreCardProps) {
  const root = process.cwd();
  const [interRegular, interExtraBold, logo, lihi, camille, atlas] = await Promise.all([
    readFile(join(root, "src/assets/fonts/inter-latin-400-normal.woff")),
    readFile(join(root, "src/assets/fonts/inter-latin-800-normal.woff")),
  ...
  const scoreColor = score >= 85 ? "#4ADE80" : score >= 60 ? "#FBBF24" : "#F87171";
  const domainSize = domain.length > 28 ? 34 : domain.length > 18 ? 44 : 54;
    ...
    (
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          ...
          >
            <div style={{ display: "flex", fontSize: 132, fontWeight: 800, color: scoreColor, lineHeight: 1 }}>
              {score}
            </div>
            <div style={{ display: "flex", fontSize: 44, fontWeight: 800, color: "#8E8598", marginLeft: 8 }}>
              /100
            </div>
            <div
              style={{
          ...
          >
            llms.txt · AI crawler access · structured data · Bing indexability
          </div>
        </div>

        <div style={{ display: "flex", fontSize: 24, color: "#8E8598" }}>
          tripleandco.com/ai-visibility-checker
        </div>
      </div>
    ),
    {
